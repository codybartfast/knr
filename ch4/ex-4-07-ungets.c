...
/*
 * My default preference is to use ungetc.  This so we can minimize the code
 * that needs to know the details of the implementation (buff, buffp). This
 * should make the code more robust.  If we knew performace was important
 * and that accessing buff, buffp materially improved perfromance, then it
 * might be best to access them directly.
 *
 * Ungets first calls ungetch for the last char in s.  This is so that
 * subsequent calls to getch will recreate the orignal string, not its
 * reverse.
 */
